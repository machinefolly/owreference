...
function Linkified({ text, quiet }){
  ...
  while ((m = LINK_REGEX.exec(text)) !== null){
    ...
    parts.push(<a key={m.index} className={'lnk ' + (quiet?'lnk--quiet':'')} href="#" onClick={e=>e.preventDefault()}>{m[0]}</a>);
      ...
      <div className="hdr__inner">
        <a className="hdr__brand" href="#" onClick={e=>{e.preventDefault(); setActiveTab('Index');}}>
          <div className="hdr__mark" aria-hidden="true">
            <svg viewBox="0 0 32 32" width="22" height="22"><path fill="currentColor" d="M16 2L3 8v9c0 7.4 5.5 12.5 13 13 7.5-.5 13-5.6 13-13V8L16 2zm0 2.2L26.8 9 16 13.8 5.2 9 16 4.2zM5 10.8l10 4.5v12.6c-6-.8-10-5.2-10-11.4v-5.7zm12 17.1V15.3l10-4.5v5.7c0 6.2-4 10.6-10 11.4z"/></svg>
          </div>
          <div className="hdr__title-wrap">
            <span className="hdr__sub">Old World</span>
            <span className="hdr__title">Reference</span>
          </div>
        </a>
...
function Cell({ row, nation, value, t, hoverCol, setHoverCol, hoverRow, setHoverRow, ri, ci }){
  ...
  const quietLinks = !t.linkifyAlways;

  if (row.kind === 'shrine'){
    const sh = value;
    const typeLower = (sh.type||'').toLowerCase();
    return (
      <td className={cls} onMouseEnter={onEnter}>
        <div className="shrine">
          <span className={`shrine__type shrine__type--${typeLower}`}>
            {t.shrineArt && typeLower ? (
              <img className="shrine__art" src={`public/img/icons/shrines/${typeLower}.png`} alt=""
        ...
        <div className="cell__body"><Linkified text={sh.effect} quiet={quietLinks}/></div>
  ...
  if (row.kind === 'family'){
    return (
      <td className={cls} onMouseEnter={onEnter}>
        <div className="fam">
          <span className="fam__class"><Linkified text={value.class} quiet={quietLinks}/></span>
  ...
  return (
    <td className={cls} onMouseEnter={onEnter}>
      <div className="cell__body"><Linkified text={String(value)} quiet={quietLinks}/></div>
      {t.showYieldIcons && yKey && <span className="cell__yico" aria-hidden>{YIELD_GLYPH[yKey]}</span>}
    </td>
  );
}

/* ─── Nation picker (the &quot;X of Y visible&quot; pill) ─── */
function NationPicker({ ordered, picked, setPicked }){
  ...
  return (
    <div className="picker" ref={ref}>
      <button type="button" className="stat stat--button" aria-expanded={open}
              onClick={() => setOpen(o => !o)}>
        <b>{picked.size}</b> of <b>{ordered.length}</b> visible
        <span className="stat__chev">{open ? '▴' : '▾'}</span>
      </button>
      {open && (
        <div className="picker__panel" role="dialog" aria-label="Filter nations">
          <div className="picker__actions">
            <button type="button" className="picker__action"
                    onClick={() => setPicked(new Set(ordered.map(n => n.slug)))}>Select all</button>
            <button type="button" className="picker__action"
                    onClick={() => setPicked(new Set())}>Clear</button>
            <button type="button" className="picker__action picker__action--close"
            ...
            {ordered.map(n => (
              <li key={n.slug}>
                <label className="picker__row">
                  <input type="checkbox" className="picker__cb"
                         checked={picked.has(n.slug)}
                         onChange={() => toggle(n.slug)}/>
                  <img className="picker__crest" src={`public/img/crests/${n.slug}.png`} alt=""
                       onError={(e)=>{ e.target.style.display='none'; }}/>
                  <span className="picker__name">{n.name}</span>
                </label>
              </li>
            ))}
          </ul>
...
/* ─── Table ─── */
function NationsTable({ t, search, picked }){
  ...
  const matches = (n) => {
    if (!picked.has(n.slug)) return false;
    if (!q) return true;
...
/* ─── Legends ─── */
function Legends({ variant }){
  const yields = ['orders','science','civics','culture','money','training','food','growth','wood','stone','iron'];
  const explanation = variant === 'tufte'
    ? <>Left edge = yield given. Right edge = nation tint.</>
    : <>Cell color = what a bonus <em>gives</em>. Left edge = nation tint.</>;
  return (
    <div className="legends">
      ...
      <div className="legend">
        <span className="legend__label">{explanation}</span>
      </div>
    </div>
  );
}
...
function App(){
  ...
  const orderedNations = useMemo(() => [...NATIONS].sort((a,b)=>a.name.localeCompare(b.name)), []);
  const [picked, setPicked] = useState(() => new Set(orderedNations.map(n => n.slug)));
    ...
    <div className={`app app--${t.variant} app--${t.density}`}>
      ...
      <main className="main">
        <div className="page-meta">
          <div>
            <span className="page-eyebrow">All 13 · Side by side</span>
            <h1 className="page-title"><span className="page-title__mark">👑</span>Nations</h1>
          </div>
          <div className="page-stats">
            <NationPicker ordered={orderedNations} picked={picked} setPicked={setPicked}/>
          </div>
        </div>
        <NationsTable t={t} search={search} picked={picked}/>
        <Legends variant={t.variant}/>
      ...
      <TweaksPanel title="Tweaks">
        <TweakSection title="Direction">
          <TweakSelect
            ...
            options={[
              { value: 'tufte',       label: 'Tufte — maximize data-ink' },
              { value: 'refined',     label: 'Refined — polished current' },
              { value: 'columnar',    label: 'Columnar — strong nation columns' },
              { value: 'engraved',    label: 'Engraved — atmospheric, embossed' },
            ]}
          />
          <p className="tw-help">
            <b>Tufte</b> reduces color from full-cell fills to a 3 px edge bar — data, not decoration. Type does the rest of the work.
          </p>
        </TweakSection>
        ...
        <TweakSection title="Reading aids">
          ...
          <TweakToggle label="Always-on link underlines" value={t.linkifyAlways} onChange={v=>setTweak('linkifyAlways', v)}/>
          <TweakToggle label="Shrine game art (vs glyph)" value={t.shrineArt} onChange={v=>setTweak('shrineArt', v)}/>
          <TweakToggle label="Show nation crest in header" value={t.showCrest} onChange={v=>setTweak('showCrest', v)}/>
        </TweakSection>
      </TweaksPanel>
    </div>
  );
